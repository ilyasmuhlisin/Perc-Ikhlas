  ...
  try {
    if (req.params.category !== "Choose category") {
      const categoryExists = await Category.findOne({
        // balikan encode
        // encodeURIComponent membaca nama kategor "undangan/ulem" = undangan%Fulem
...
const saveAttr = async (req, res, next) => {
  ...
  try {
    // undangan/ulem = undangan
    const category = categoryChoosen.split("/")[0];
    // mencara apakah kategpri ada
    const categoryExists = await Category.findOne({ name: category }).orFail();
    if (categoryExists.attrs.length > 0) {
      // jika key ada tambahkan value ke key
      ...
      categoryExists.attrs.map((item, idx) => {
        if (item.key === key) {
          ...
          var newAttributeValues = [...new Set(copyAttributeValues)]; // new set(["a","b","b"]) = {"a","b"} tanpa pengulangan
          categoryExists.attrs[idx].value = newAttributeValues; // ditimpa
        }
      });
