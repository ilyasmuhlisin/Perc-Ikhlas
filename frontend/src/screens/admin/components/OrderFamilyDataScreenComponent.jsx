...
function OrderFamilyDataScreenComponent({ getFamiliesDataOrder }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    getFamiliesDataOrder(id)
      .then((order) => {
        // console.log(order.families);
        setMaleName(order.families.maleName);
        setMaleParent(order.families.maleParent);
        setMaleAddress(order.families.maleAddress);
        setReceptionDate(order.families.receptionDate);
        setReceptionPlace(order.families.receptionPlace);
        setFemaleName(order.families.femaleName);
        setFemaleParent(order.families.femaleParent);
        setFemaleAddress(order.families.femaleAddress);
        setAgreementDate(order.families.agreementDate);
        setAgreementPlace(order.families.agreementPlace);
      })
      .catch(
        (er) => dispatch(logout())
        //   console.log(
...
export default OrderFamilyDataScreenComponent;
